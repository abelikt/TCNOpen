...
#--------------------------------------------------------
# (forget and) resolve path to the TSN destination IP address (using ping):

echo ""
echo "Resolve MAC-address for IP $DEST_TSN_IP"
# arp -d $DEST_TSN_IP  >/dev/null 2>&1;
ping -c 1 $DEST_TSN_IP  >/dev/null 2>&1;
arp $DEST_TSN_IP | sed -r "s/^.*(([0-9]{2}:){5}[0-9]{2}).*|.*$/\1/" | sort -r
